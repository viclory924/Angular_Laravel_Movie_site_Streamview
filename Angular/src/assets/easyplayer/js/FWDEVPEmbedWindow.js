...
(function (window){
	...
	var FWDEVPEmbedWindow = function(data, parent){
		...
		self.useVectorIcons_bl = data.useVectorIcons_bl
	
		//#################################//
		/* init */
		//#################################//
		this.init = function(){
			if(self.closeButton_do) return;
			...
			self.copyLinkButton_do = new FWDEVPSimpleSizeButton(
			...
			self.copyLinkButton_do.screen.style.position = 'absolute';
			self.copyLinkButton_do.addListener(FWDEVPSimpleSizeButton.CLICK, function(){
				self.copyToClipboard(self.linkText_do.screen);
			});

			FWDEVPSimpleSizeButton.setPrototype();
			self.copyEmbedButton_do = new FWDEVPSimpleSizeButton(
					data.embedCopyButtonNPath_str,
					data.embedCopyButtonSPath_str,
					self.buttonWidth,
					self.buttonHeight,
			...
			self.copyEmbedButton_do.screen.style.position = 'absolute';
			self.copyEmbedButton_do.addListener(FWDEVPSimpleSizeButton.CLICK,function(){
				self.copyToClipboard(self.embedText_do.screen);
			});
			...
			if(self.useVectorIcons_bl){
				...
				self.closeButton_do = new FWDEVPSimpleButton(
						undefined, undefined, undefined, true, undefined, undefined, undefined,
						"<div class='table-fwdevp-button'><span class='table-cell-fwdevp-button icon-close'></span></div>",
						undefined,
				...
				FWDEVPSimpleButton.setPrototype();
				self.closeButton_do = new FWDEVPSimpleButton(data.embedClooseN_img, data.embedWindowClosePathS_str, undefined,
						true,
						data.useHEXColorsForSkin_bl,
						data.normalButtonsColor_str,
						data.selectedButtonsColor_str);
		...
		this.updateHEXColors = function(normalColor_str, selectedColor_str){
			if(!self.copyEmbedButton_do) return;
		...
		this.show = function(id){
			...
			if(self.useVectorIcons_bl){
				self.checkButtonsId_to = setInterval(function(){
					
					if(self.closeButton_do.w != 0){	
						self.positionAndResize();
						
						clearInterval(self.checkButtonsId_to);
