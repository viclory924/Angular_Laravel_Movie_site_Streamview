...
var FWDEVPComplexButton = function(
		...
		self.setupMainContainers = function(){
			if(self.useFontAwesome_bl){
				...
				self.n1_do.setInnerHTML(self.iconCSSString);
				...
				self.n2_do.setInnerHTML(self.icon2CSSString);
		...
		self.onMouseOver = function(e, animate){
			...
			if(!e.pointerType || e.pointerType == e.MSPOINTER_TYPE_MOUSE || e.pointerType == "mouse"){
				...
				self.dispatchEvent(FWDEVPComplexButton.SHOW_TOOLTIP, {e:e});
		...
		this.setFinalSize = function(){
			...
			self.lastWidth = self.n1_do.screen.firstChild.offsetWidth;
			...
			var maxWidth = Math.max(self.n1_do.screen.firstChild.offsetWidth, self.n2_do.screen.firstChild.offsetWidth); 
			var maxHeight = Math.max(self.n1_do.screen.offsetHeight, self.n2_do.screen.firstChild.offsetHeight); 
			self.buttonWidth = maxWidth;
			self.buttonHeight = maxHeight;
			
			self.setWidth(maxWidth);
			self.setHeight(maxHeight);
			self.firstButton_do.setWidth(self.w);
			self.firstButton_do.setHeight(self.h);
			self.secondButton_do.setWidth(self.w);
			self.secondButton_do.setHeight(self.h);
			
			self.n1_do.setX(Math.round((maxWidth - self.n1_do.getWidth())/2));
			self.n1_do.setY(Math.round((maxHeight - self.n1_do.getHeight())/2));
			self.n2_do.setX(Math.round((maxWidth - self.n2_do.getWidth())/2));
			self.n2_do.setY(Math.round((maxHeight - self.n2_do.getHeight())/2));
		...
		this.setNormalState = function(animate){
			if(self.isMobile_bl && !self.hasPointerEvent_bl && !self.useFontAwesome_bl) return;
			self.isSelectedState_bl = false;
			FWDAnimation.killTweensOf(self.s1_do);
			FWDAnimation.killTweensOf(self.s2_do);
				
			if(self.useFontAwesome_bl){
				FWDAnimation.killTweensOf(self.n1_do.screen);
				FWDAnimation.killTweensOf(self.n2_do.screen);
					
				if(animate){
					FWDAnimation.to(self.n1_do.screen, .8, {className:self.normalCalssName, ease:Expo.easeOut});	
					FWDAnimation.to(self.n2_do.screen, .8, {className:self.normalCalssName, ease:Expo.easeOut});
				}else{
					self.n1_do.screen.className = self.normalCalssName;
					self.n2_do.screen.className = self.normalCalssName;
				}
			}else{
		...
		this.setSelectedState = function(animate){
			...
			if(self.useFontAwesome_bl){
					...
					FWDAnimation.killTweensOf(self.n2_do.screen);
					if(animate){
						FWDAnimation.to(self.n1_do.screen, .8, {className:self.selectedCalssName, ease:Expo.easeOut});	
						FWDAnimation.to(self.n2_do.screen, .8, {className:self.selectedCalssName, ease:Expo.easeOut});	
					}else{
						self.n1_do.screen.className = self.selectedCalssName;
						self.n2_do.screen.className = self.selectedCalssName;
					}
			}else{
