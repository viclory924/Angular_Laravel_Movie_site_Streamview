...
var FWDEVPController = function(
		...
		this.showRewindButton_bl = data.showRewindButton_bl;
		...
		this.useVectorIcons_bl = data.useVectorIcons_bl
		...
		self.init = function(){
			...
			if(data.useAToB) self.setupATB();
			
			if(data.thumbnailsPreview) self.setupThumbnailsPreview();
			if(self.repeatBackground_bl){
				self.bk_do = new FWDEVPDisplayObject("div"); 
				self.bk_do.getStyle().background = "url('" + self.controllerBkPath_str +  "')";
			}else{
			...
			if(self.showYoutubeQualityButton_bl){
				self.ytbQuality_ar = ["hd2160", "hd2160", "hd1440", "highres", "hd1080", "hd720", "large", "medium", "small", "tiny"];
				self.ytbButtons_ar = [];
			...
			if(self.showRewindButton_bl) self.setupRewindButton();
			...
			if(data.useAToB) self.setupAtbButton();
		...
		self.positionButtons = function(){
			...
			if(parent.videoType_str == FWDEVPlayer.VIMEO && !data.showDefaultControllerForVimeo_bl){
				self.setX(-5000);
			}else{
				self.setX(0);
			}
			...
			while(self.mainScrubberWidth < self.mainScrubberMinWidth && buttonsCopy_ar.length > testLegnth){
				...
				}else if(self.atbButton_do && FWDEVPUtils.indexOfArray(buttonsCopy_ar, self.atbButton_do) != -1){
					buttonsCopy_ar.splice(FWDEVPUtils.indexOfArray(buttonsCopy_ar, self.atbButton_do), 1);
					self.atbButton_do.setX(-1000);
			...
			for (var i=0; i < buttonsCopy_ar.length; i++) {
				...
				if(i == 0){
					...
					var timeOffset = 0;
					if(self.isLive) timeOffset = 2;
					button.setY(parseInt((self.stageHeight - button.h)/2) + timeOffset);
		...
		this.positionScrollBarOnTopOfTheController = function(){
			...
			var offset = 0;
			if(self.atb && self.atb.isShowed_bl) offset = self.stageHeight + 1;
			if(data.showScrubberWhenControllerIsHidden_bl){
				if(self.isMainScrubberOnTop_bl || self.isShowed_bl){
					self.mainScrubber_do.setY(- self.mainScrubberOffestTop - offset);
				}else if(self.mainScrubber_do.y != - self.mainScrubberOffestTop && !self.isLive){
					FWDAnimation.to(self.mainScrubber_do, .8, {y:- self.mainScrubberOffestTop - offset, ease:Expo.easeOut});
				}
			}
		...
		this.setupThumbnailsPreview =  function(){
			...
			self.thumbnailsPreview_do = new FWDEVPThumbnailsPreview(self);
			self.thumbnailsPreview_do.addListener(FWDEVPData.LOAD_ERROR, function(e){
				self.dispatchEvent(FWDEVPData.LOAD_ERROR, {text:e.text});
			});
		}
		...
		this.setupATB = function(){
			...
			self.atb = new FWDEVPATB(self);
			self.mainHolder_do.addChild(self.atb);
			self.atb.addListener(FWDEVPATB.START_TO_SCRUB, self.atbStartToScrub);
			self.atb.addListener(FWDEVPATB.STOP_TO_SCRUB, self.atbStopToScrub);
		}

		self.atbStartToScrub = function(){
			parent.showDisable();
		}

		self.atbStopToScrub = function(){
			parent.hideDisable();
		}
		...
		this.setupAtbButton = function(){
			if(self.useVectorIcons_bl){
				...
				self.atbButton_do = new FWDEVPSimpleButton(
						undefined, undefined, undefined, true, undefined, undefined, undefined,
						"<span class='icomoon icon-AB'></span>",
						undefined,
			...
			self.atbButton_do.setX(-5000);
			self.buttons_ar.push(self.atbButton_do);
			self.atbButton_do.setY(parseInt((self.stageHeight - self.atbButton_do.h)/2));
			var checkIconInterval = setInterval(function(){
				if(self.atbButton_do.buttonHeight > 0){
					clearInterval(checkIconInterval);
					self.atbButton_do.setY(parseInt((self.stageHeight - self.atbButton_do.buttonHeight)/2));
				}
			}, 50);
			self.atbButton_do.addListener(FWDEVPSimpleButton.SHOW_TOOLTIP, self.atbButtonShowTooltipHandler);
			self.atbButton_do.addListener(FWDEVPSimpleButton.MOUSE_UP, self.atbButtonMouseUpHandler);
			self.mainHolder_do.addChild(self.atbButton_do);
		};
		
		this.atbButtonShowTooltipHandler = function(e){
			//self.showToolTip(self.atbButton_do, self.atbButtonToolTip_do, e.e);
		};
		
	
		this.atbButtonMouseUpHandler = function(){
			if(self.atbButton_do.isSelected){
				self.atbButton_do.doNotallowToSetNormal = false;
				self.atbButton_do.isSelected = false;
				self.atb.hide(true);
			}else{
				self.atbButton_do.isSelected = true;
				self.atbButton_do.doNotallowToSetNormal = true;
				self.atbButton_do.setSelectedState();
				self.atb.show(true);
			}
		...
		this.disableAtbButton = function(){
			if(self.atbButton_do) self.atbButton_do.disable();
		};
		
		this.enableAtbButton = function(){
			if(self.atbButton_do) self.atbButton_do.enable();
		};
		...
		this.setupPlaybackRateButton = function(){
			if(self.useVectorIcons_bl){
				FWDEVPSimpleButton.setPrototype();
				self.playbackRateButton_do = new FWDEVPSimpleButton(
						undefined, undefined, undefined, true, undefined, undefined, undefined,
						"<span class='icomoon icon-watch-later'></span>",
						undefined,
						"EVPMainButtonsNormalState",
						"EVPMainButtonsSelectedState"
				);
		...
		this.positionAdsLines = function(totalDuration){
			...
			if(this.line_ar){
				...
				for(var i=0; i<this.line_ar.length; i++){
					line = this.line_ar[i];
					var lineX = Math.round((line.timeStart/self.totalDuration) * self.mainScrubberWidth) - 1;
		...
		/* Set is live */
		//###############################################//
		self.setIsLive = function(isLive){
			self.isLive = isLive;
			if(isLive){
				if(!self.mainScrubber_do.contains(self.live_do)){
					self.mainScrubber_do.setAlpha(.2);
					self.mainHolder_do.addChild(self.live_do);
					setTimeout(function(){
							self.live_do.setX(4);
							self.live_do.setY(- self.live_do.getHeight() - 4);
					}, 100)
					self.disableMainScrubber();
				}
			}else{
				if(self.mainHolder_do.contains(self.live_do)){
					self.mainHolder_do.removeChild(self.live_do);
					self.mainScrubber_do.setAlpha(1);
					
					self.enableMainScrubber();
				}
			}
		}
		...
		this.setupMainScrubber = function(){
			...
			self.live_do = new FWDEVPDisplayObject("div");
			self.live_do.hasTransform3d_bl = false;
			self.live_do.hasTransform2d_bl = false;
			self.live_do.setBackfaceVisibility();
			self.live_do.getStyle().fontFamily = "Arial";
			self.live_do.getStyle().fontSize= "12px";
			self.live_do.getStyle().whiteSpace= "nowrap";
			self.live_do.getStyle().textAlign = "center";
			self.live_do.getStyle().padding = "4px";
			self.live_do.getStyle().paddingLeft = "6px";
			self.live_do.getStyle().paddingRIght = "6px";
			self.live_do.getStyle().color = "#FFFFFF";
			self.live_do.getStyle().fontSmoothing = "antialiased";
			self.live_do.getStyle().webkitFontSmoothing = "antialiased";
			self.live_do.getStyle().textRendering = "optimizeLegibility";
			self.live_do.getStyle().backgroundColor = "rgba(255,0,0,0.8)";
			self.live_do.setInnerHTML("&#x25C9; LIVE");
			...
			if(!self.disableVideoScrubber_bl){
				if(self.hasPointerEvent_bl){
					...
					if(!self.isMobile_bl){
						...
						self.mainScrubber_do.screen.addEventListener("mousemove", self.updateTooltipOnMove);
			...
			self.ttm = new FWDEVPScrubberToolip(self.mainScrubber_do, data.scrubbersToolTipLabelBackgroundColor, data.scrubbersToolTipLabelFontColor, test);
			self.addChild(self.ttm);

		};

		this.updateToolTip = function(localX, percentScrubbed){
			if(!data.showMainScrubberToolTipLabel_bl) return;
			self.ttm.setLabel(FWDEVPUtils.formatTime(Math.round(parent.totalDuration * percentScrubbed)));
			self.ttm.setX(Math.round(self.mainScrubber_do.x + localX - self.ttm.getWidth()/2) + 1);
			self.ttm.setY(self.mainScrubber_do.y - self.ttm.h - 2);
		}
		
		this.updateThumbnailsPreview = function(localX, percentScrubbed){
			if(!data.thumbnailsPreview) return;
			
			var x = Math.round(self.mainScrubber_do.x + localX - self.thumbnailsPreview_do.getWidth()/2) + 1;
			if(x < 1){
				x = 1;
			}else if(x > self.stageWidth - self.thumbnailsPreview_do.w){
				x = self.stageWidth - self.thumbnailsPreview_do.w;
			}
			self.thumbnailsPreview_do.setLabel(FWDEVPUtils.formatTime(Math.round(parent.totalDuration * percentScrubbed)), Math.round(parent.totalDuration * percentScrubbed));
			self.thumbnailsPreview_do.setX(x);
			self.thumbnailsPreview_do.setY(self.mainScrubber_do.y - self.thumbnailsPreview_do.h - 2);
		}
		...
		this.updateTooltipOnMove = function(e){
			...
			self.updateToolTip(localX, percentScrubbed);
			self.updateThumbnailsPreview(localX, percentScrubbed);
		}
		
		this.mainScrubberOnOverHandler =  function(e){
			if(self.isMainScrubberDisabled_bl) return;

			if(data.showMainScrubberToolTipLabel_bl) self.ttm.show();
			if(data.thumbnailsPreview) self.thumbnailsPreview_do.show();
		...
		this.mainScrubberOnOutHandler =  function(e){
			if(self.isMainScrubberDisabled_bl) return;
			if(!self.isMainScrubberScrubbing_bl){
				if(self.ttm) self.ttm.hide();
				if(self.thumbnailsPreview_do) self.thumbnailsPreview_do.hide();
			}
		};
		...
		this.mainScrubberEndHandler = function(e){
			self.isMainScrubberScrubbing_bl = false;
			if(self.disable_do){
			...
			*/
			...
			if(e){
				...
				if(!FWDEVPUtils.hitTest(self.mainScrubber_do.screen, wp.screenX, wp.screenY)){
					if(self.ttm) self.ttm.hide();
					if(self.thumbnailsPreview_do) self.thumbnailsPreview_do.hide();
				}
			}
		...
		this.enableMainScrubber = function(){
			if(!self.mainScrubber_do || self.isLive) return;
		...
		this.setupPlayPauseButton = function(){
			if(self.useVectorIcons_bl){
				...
				self.playPauseButton_do = new FWDEVPComplexButton(undefined, undefined, undefined, undefined, true, undefined, undefined, undefined,
		...
		this.showPlayButton = function(){
			if(!self.playPauseButton_do || (!data.showDefaultControllerForVimeo_bl && parent.videoType_str == FWDEVPlayer.VIMEO)) return;
		...
		this.setupEmbedButton = function(){
			...
			if(self.useVectorIcons_bl){			
				FWDEVPSimpleButton.setPrototype();
				self.embedButton_do = new FWDEVPSimpleButton(
						undefined, undefined, undefined, true, undefined, undefined, undefined,
		...
		this.setupSubtitleButton = function(){
			if(self.useVectorIcons_bl){
				FWDEVPComplexButton.setPrototype();
				self.subtitleButton_do = new FWDEVPComplexButton(undefined, undefined, undefined, undefined, true, undefined, undefined, undefined,
					"<span class='icomoon icon-CC'></span>",
					"<span class='icomoon icon-CC-off'></span>",
					"EVPMainButtonsNormalState",
					"EVPMainButtonsSelectedState"
				);
			}else{
			...
			self.subtitleButton_do.setX(-10000);
		...
		this.setupRewindButton = function(){
			if(self.useVectorIcons_bl){				
				FWDEVPSimpleButton.setPrototype();
				self.rewindButton_do = new FWDEVPSimpleButton(
						undefined, undefined, undefined, true, undefined, undefined, undefined,
						"<span class='icomoon icon-10'></span>",
						undefined,
						"EVPMainButtonsNormalState",
						"EVPMainButtonsSelectedState"
				);
			...
			self.buttons_ar.push(self.rewindButton_do);
			self.rewindButton_do.setY(parseInt((self.stageHeight - self.rewindButton_do.h)/2));
			self.rewindButton_do.addListener(FWDEVPSimpleButton.MOUSE_UP, self.rewindButtonMouseUpHandler);
			self.mainHolder_do.addChild(self.rewindButton_do);
		};
		
	
		this.rewindButtonMouseUpHandler = function(){
			self.dispatchEvent(FWDEVPController.REWIND);
		};
		...
		this.setupDownloadButton = function(){
			if(self.useVectorIcons_bl){
				FWDEVPSimpleButton.setPrototype();
				self.downloadButton_do = new FWDEVPSimpleButton(
						undefined, undefined, undefined, true, undefined, undefined, undefined,
		...
		this.setupFullscreenButton = function(){
			...
			if(self.useVectorIcons_bl){
				FWDEVPComplexButton.setPrototype();
				self.fullScreenButton_do = new FWDEVPComplexButton(undefined, undefined, undefined, undefined, true, undefined, undefined, undefined,
					"<span class='icomoon icon-fullscreen'></span>",
					"<span class='icomoon icon-normalscreen'></span>",
					"EVPMainButtonsNormalState",
					"EVPMainButtonsSelectedState"
				);
			}else{
		...
		this.setNormalStateToFullScreenButton = function(){
			if(!self.fullScreenButton_do) return;
			self.fullScreenButton_do.setNormalState(true);
			self.hideQualityButtons(false);
		};
		...
		this.updateTime = function(time){
			...
			if(self.isLive) time = time.substr(0, time.indexOf("/"));
		...
		this.setupVolumeButton = function(){
			if(self.useVectorIcons_bl){
				...
				self.volumeButton_do = new FWDEVPVolumeButton(undefined, undefined, undefined, undefined, undefined, undefined,
					"<span class='icomoon icon-sound'></span>",
					"<span class='icomoon icon-sound-off'></span>",
					"EVPMainButtonsNormalState",
					"EVPMainButtonsSelectedState"
				);
			}else{
			...
			self.volumeButton_do.setX(-10000);
		...
		this.hide = function(animate, hideForGood){
			...
			if(self.atb && self.atb.isShowed_bl) offsetY += self.h + 1;
			if(animate){
		...
		self.updateHEXColors = function(normalColor_str, selectedColor_str){
			...
			if(self.rewindButton_do) self.rewindButton_do.updateHEXColors(normalColor_str, selectedColor_str);
