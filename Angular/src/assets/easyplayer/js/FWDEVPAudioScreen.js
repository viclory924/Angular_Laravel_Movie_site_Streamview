	...
	var FWDEVPAudioScreen = function(parent, volume){
		...
		this.updateAudio = function(){
			...
			if(!isNaN(self.audio_el.duration)){
				self.dispatchEvent(FWDEVPAudioScreen.UPDATE_TIME, {curTime: curTime, totalTime:totalTime, seconds:Math.round(self.audio_el.currentTime), totalTimeInSeconds:self.audio_el.duration});
			}else{
		...
		this.updateSubtitleHandler = function(){
			self.dispatchEvent(FWDEVPAudioScreen.UPDATE_SUBTITLE, {curTime:self.audio_el.currentTime});
		}
		...
		this.setupSpectrum = function(){
			if(parent.useWithoutVideoScreen_bl) return;
