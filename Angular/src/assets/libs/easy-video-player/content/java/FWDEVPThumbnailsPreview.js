...
var FWDEVPThumbnailsPreview = function(
		...
		this.onLoad = function(e){
			...
			if(self.xhr.readyState == 4){
				if(self.xhr.status == 404){
					self.dispatchEvent(FWDEVPData.LOAD_ERROR, {text:"Thumbnails preview .vtt file not found: <font color='#FF0000'>" + self.sourceURL_str + "</font>"});
				}else if(self.xhr.status == 408){
					self.dispatchEvent(FWDEVPData.LOAD_ERROR, {text:"Loadiong thumbnails preview .vtt file file file request load timeout!"});
			...
			self.dispatchEvent(FWDEVPThumbnailsPreview.LOAD_COMPLETE);
		};
		...
		this.onError = function(e){
			...
			self.dispatchEvent(FWDEVPThumbnailsPreview.LOAD_ERROR, {text:"Error loading thumbnails preview .vtt file : <font color='#FF0000'>" + self.sourceURL_str + "</font>."});
		};
		...
		self.parseVtt = function(file_str){
		    ...
		    for(s in srt_) {
		        ...
		        if(st.length >=2) {
		            ...
		            self.vtt_ar[cont].startDuration = FWDEVPUtils.formatTimeWithMiliseconds(self.vtt_ar[cont].start);
		            self.vtt_ar[cont].endDuration = FWDEVPUtils.formatTimeWithMiliseconds(self.vtt_ar[cont].end);
		        }
		        cont++;
		    }
	...
	FWDEVPThumbnailsPreview.setPrototype = function(){
		...
		FWDEVPThumbnailsPreview.prototype = new FWDEVPTransformDisplayObject("div");
	};
