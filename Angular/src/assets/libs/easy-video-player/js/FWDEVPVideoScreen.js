	...
	var FWDEVPVideoScreen = function(parent, backgroundColor_str, volume){
		...
		this.setupVideo = function(){
			if(self.video_el == null){
				...
				if(parent.displayType ==  FWDEVPlayer.BACKGROUND_VIDEO) self.video_el.muted = true;
				if(!parent.data.playsinline){
		...
		this.updateVideo = function(){
			...
			if(!isNaN(self.video_el.duration)){
				self.dispatchEvent(FWDEVPVideoScreen.UPDATE_TIME, {curTime: curTime, totalTime:totalTime, seconds:parseInt(self.video_el.currentTime), totalTimeInSeconds:self.video_el.duration});
			}else{
		...
		this.updateSubtitleHandler = function(){
			self.dispatchEvent(FWDEVPVideoScreen.UPDATE_SUBTITLE, {curTime:self.video_el.currentTime});
		}
