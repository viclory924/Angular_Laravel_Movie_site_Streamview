...
var FWDEVPVolumeButton = function(
		...
		this.iconCSSString1 = iconCSSString1;
		this.iconCSSString2 = iconCSSString2;
		this.nImg = nImg;
		...
		self.setupMainContainers = function(){
			...
			if(self.useFontAwesome_bl){
				...
				self.n_sdo.setInnerHTML(self.iconCSSString1);
				self.addChild(self.n_sdo);
				
				self.d_sdo = new FWDEVPTransformDisplayObject("div");
				self.d_sdo.setInnerHTML(self.iconCSSString2);
		...
		this.setFinalSize = function(){
			...
			self.setWidth(self.n_sdo.getWidth());
			self.setHeight(self.n_sdo.getHeight());
		...
		this.setNormalState = function(animate){
			if(self.useFontAwesome_bl){
				FWDAnimation.killTweensOf(self.n_sdo.screen);
				FWDAnimation.killTweensOf(self.d_sdo.screen);
				if(animate){
					FWDAnimation.to(self.n_sdo.screen, .8, {className:self.normalCalssName, ease:Expo.easeOut});
					FWDAnimation.to(self.d_sdo.screen, .8, {className:self.normalCalssName, ease:Expo.easeOut});
				}else{
					self.n_sdo.screen.className = self.normalCalssName;
					self.d_sdo.screen.className = self.normalCalssName;
				}
			}else{
		...
		this.setSelectedState = function(animate){
			if(self.useFontAwesome_bl){
				...
				FWDAnimation.killTweensOf(self.d_sdo.screen);
				if(animate){
					FWDAnimation.to(self.n_sdo.screen, .8, {className:self.selectedCalssName, ease:Expo.easeOut});	
					FWDAnimation.to(self.d_sdo.screen, .8, {className:self.selectedCalssName, ease:Expo.easeOut});
				}else{
					self.n_sdo.screen.className = self.selectedCalssName;
					self.d_sdo.screen.className = self.selectedCalssName
				}
			}else{
				FWDAnimation.killTweensOf(self.s_sdo);
		...
		this.setDisabledState = function(){
			...
			if(self.useFontAwesome_bl){
				self.n_sdo.setX(-10000);
			}else{self.d_sdo.setX(0);
