...
export class SearchVideoComponent implements OnInit {
  ...
  ) { this.titleService.setTitle(AppGlobal.title+' | Search')}
		...
		.subscribe(data=>{
			if(data.success){
				...
				if (data.data.length > 0) {
					...
					this.no_results_found = "img/no-results.png";
				}
...
showVideoDrop = function(event, idx, key) {console.log('video_drop',idx, key)
