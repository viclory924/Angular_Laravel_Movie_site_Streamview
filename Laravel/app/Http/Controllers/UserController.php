...
class UserController extends Controller {
    ...
    {
        $this->UserAPI = $API;
        $this->middleware('auth', ['except' => ['index','single_video','all_categories' ,'category_videos' , 'sub_category_videos' , 'contact','trending', 'embed_video', 'genre_embed_video']]);
    }
    /**
    ...
    public function single_video($id) {
        ...
        if($video) {
            $original_trailer_video = $original_main_video = $hls_video = $hls_main_video = $hls_trailer_video = "";
            ...
            if ($video->video_type == 1) {
                $hls_main_video = Setting::get('HLS_STREAMING_URL').get_video_end($video->video);
                $hls_trailer_video = Setting::get('HLS_STREAMING_URL').get_video_end($video->trailer_video);
                if (\Setting::get('streaming_url')) {
    ...
    public function history_add(Request $request) {
        ...
        $response = $this->UserAPI->history_add($request)->getData();
        if($response->success) {
    ...
    public function history_delete(Request $request) {
        ...
        $response = $this->UserAPI->history_delete($request)->getData();
        if($response->success) {
    ...
    public function wishlist_add(Request $request) {
        ...
        if($request->status == 1) {
            $response = $this->UserAPI->wishlist_add($request)->getData();
        } else {
