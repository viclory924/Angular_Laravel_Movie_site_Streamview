...
class ModeratorController extends Controller
{
    ...
    public function dashboard() {
        ...
        return view('moderator.dashboard.dashboard')
                    ->withPage('dashboard')
                    ->with('sub_page','')
                    ->with('redeem_amount', $redeem_amount)
                    ->with('total', $total)
                    ->with('today_videos' , $today_videos)
                    ->with('video_payment', $video_payment);
    }
    ...
    public function change_password(Request $request) {
        ...
        if($validator->fails()) {
            ...
            if(Hash::check($old_password,$admin->password))
            {
                ...
                return back()->with('flash_error', "Old Pasword is mismatched");
            }
        }
...
public function search(Request $request) {
    ...
    $url="https://api.themoviedb.org/3";
        try {
            ...
            $movie= file_get_contents("http://www.omdbapi.com/?s=$title&apikey=70478868&type=movie"); // omdb
    ...
    $url="https://api.themoviedb.org/3";
          try {
             // $movie = file_get_contents('https://api.themoviedb.org/3/movie/'.$id.'?api_key='.$api_key);
              $movie = file_get_contents("http://www.omdbapi.com/?i=$id&apikey=70478868&plot=full");
              $data=json_decode($movie,true);
         } catch (Exception $e) {
            return ['status' =>0, 'data'=>[]];
        }
                ...
                if ($video->video_resolutions) {
                    if ($main_video_url) {
                        ...
                        if (file_exists($inputFile)) {
                            ...
                            dispatch(new StreamviewCompressVideo($inputFile, $local_url, MAIN_VIDEO, $video->id, $file_name,0));
                    ...
                    if ($trailer_video_url) {
                        if ($trailer_video) {
                            ...
                            if (file_exists($inputFile)) {
                                ...
                                dispatch(new StreamviewCompressVideo($inputFile, $local_url, TRAILER_VIDEO, $video->id,$file_name,0));
    ...
    public function video_payments() {
        $payments = PayPerView::orderBy('created_at' , 'desc')
            ...
            ->get();
        return view('moderator.payments.video-payments')
                ->with('data' , $payments)
                ->withPage('revenue')
    ...
    public function redeems(Request $request) {
        return view('moderator.payments.redeem-request')->withPage('revenue')
                ->with('sub_page', 'redeem-index');
    }
    /**
     ...
     *
     * To save a new video as well as updated video details
     *
     * @created_by - Shobana Chandrasekar
     *
     * @updated_by - - 
    ...
    public function admin_videos_edit(Request $request) {
        ...
        if ($model) {
            $categories = Category::where('categories.is_approved' , DEFAULT_TRUE)
                ...
                ->orderBy('categories.name' , 'asc')
                ->get();
            $sub_categories = get_sub_categories($model->category_id);
