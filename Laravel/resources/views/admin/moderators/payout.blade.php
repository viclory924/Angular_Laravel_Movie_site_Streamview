...
@section('breadcrumb')
    ...
    <li><a href="{{route('admin.moderators.redeems')}}"><i class="fa fa-users"></i> {{tr('redeems')}}</a></li>
        ...
        <section class="invoice">
            ...
            <div class="row invoice-info">
                ...
                <div class="col-sm-4 invoice-col">
                    To
                    <address>
                        <strong>{{$data->moderator_details ? $data->moderator_details->name : ""}}</strong><br>
                        {{$data->moderator_details ? $data->moderator_details->address : ""}}<br>
                        Email: {{$data->moderator_details ? $data->moderator_details->email : ""}}
                    </address>
                </div>
                <!-- /.col -->
                <div class="col-sm-4 invoice-col">
            ...
            <div class="row">
                ...
                <div class="col-xs-6">
                    @if(in_array($data->redeem_request_status ,[REDEEM_REQUEST_SENT , REDEEM_REQUEST_PROCESSING]))
                        @if($data->moderator_details->email)
                        <br>
                        <br>
                        <form action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
                            ...
                            <input name = "business" value = "{{$data->moderator_details ? $data->moderator_details->email : ''}}" type = "hidden">
                            <input name = "handling_cart" value = "0" type = "hidden">
                            <input name = "currency_code" value = "USD" type = "hidden">
                            <input name = "lc" value = "GB" type = "hidden">
                            <input name = "return" value = "{{route('admin.moderators.payout.response' , ['moderator_id' => $data->moderator_id , 'redeem_request_id' => $data->redeem_request_id , 'success' => true])}}" type = "hidden">
                            <input name = "cbt" value = "Return to My Site" type = "hidden">
                            <input name = "cancel_return" value = "{{route('admin.moderators.payout.response' , ['moderator_id' => $data->moderator_id , 'redeem_request_id' => $data->redeem_request_id , 'success' => false])}}" type = "hidden">
                        ...
                        @endif
                        <form action="{{route('admin.moderators.payout.direct')}}" method="post">
                            ...
                            <input type="hidden" name="moderator_id" value="{{$data->moderator_id}}">
