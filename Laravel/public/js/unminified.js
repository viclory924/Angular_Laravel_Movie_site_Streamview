...
(function (window){
	...
	var FWDConsole = function(){
		...
		this.setupScreen = function(){
			this.main_do = new FWDSISCDisplayObject("div", "absolute");
    ...
    FWDConsole.setPrototype = function(){
    	FWDConsole.prototype = new FWDSISCDisplayObject("div", "absolute");
    };
...
(function (window){
	 ...
	 */
	var FWDSISCDisplayObject = function(type, position, overflow, display){
		...
		this.hasTransform3d_bl =  FWDSISCUtils.hasTransform3d;
		this.hasTransform2d_bl =  FWDSISCUtils.hasTransform2d;
		if(FWDSISCUtils.isFirefox || FWDSISCUtils.isIE) self.hasTransform3d_bl = false;
		if(FWDSISCUtils.isFirefox || FWDSISCUtils.isIE) self.hasTransform2d_bl = false;
		...
		self.addChild = function(e){
			if(self.contains(e)){	
				self.children_ar.splice(FWDSISCUtils.indexOfArray(self.children_ar, e), 1);
		...
		self.contains = function(e){
			if(FWDSISCUtils.indexOfArray(self.children_ar, e) == -1){
				return false;
			}else{
				return true;
			}
		...
		self.addChildAt = function(e, index){
			if(self.getNumChildren() == 0){
				...
				if(self.contains(e)){
					self.children_ar.splice(FWDSISCUtils.indexOfArray(self.children_ar, e), 1, e);
				}else{
					self.children_ar.splice(FWDSISCUtils.indexOfArray(self.children_ar, e), 0, e);
				}
			}else{
		...
		self.getChildIndex = function(child){
			if(self.contains(child)){
				return FWDSISCUtils.indexOfArray(self.children_ar, child);
			}
			return 0;
		};
...
(function (window){
	...
	var FWDSISCSlideshowPreloader = function(parent, preloaderPostion, radius, backgroundColor, fillColor, strokeSize, animDuration){
		...
		this.init = function(){
			...
			this.bkCanvas =  new FWDSISCDisplayObject("canvas");
			this.bkCanvasContext = this.bkCanvas.screen.getContext('2d');
			this.fillCircleCanvas = new FWDSISCDisplayObject("canvas");
		...
		*/
		this.positionAndResize = function(){
			...
			if(self.preloaderPostion == 'bottomleft'){
				self.setX(10);
				self.setY(parent.stageHeight - self.h - 10 - Math.round(offsetY));
				...
				if(parent.cnt_do){
					...
					if(cnt_do.x > cntW){
						...
						self.setX(parent.stageWidth - Math.abs(cnt_do.x - cntW) - self.w - 6);
					}
				...
				self.setY(Math.round(parent.stageHeight - self.h)/2 - Math.round(offsetY));
			}
		}	
		...
		this.startPreloader = function(){
			...
			FWDAnimation.to(self.screen, self.animDuration, {rotation:360, transformOrigin:"50% 50%", repeat:100});
		}
		...
		this.onHideComplete = function(){
			self.setVisible(false);
			self.stopPreloader();
			self.dispatchEvent(FWDSISCSlideshowPreloader.HIDE_COMPLETE);
		};
    ...
    FWDSISCSlideshowPreloader.prototype = null;
...
(function (window){
	...
	var FWDSISCTransformDisplayObject = function(type, position, overflow, display){
		...
		this.addChild = function(e){
			if(this.contains(e)){	
				this.children_ar.splice(FWDSISCUtils.indexOfArray(this.children_ar, e), 1);
		...
		this.contains = function(e){
			if(FWDSISCUtils.indexOfArray(this.children_ar, e) == -1){
				return false;
			}else{
				return true;
			}
		...
		this.addChildAtZero = function(e){
			if(this.numChildren == 0){
				...
				if(this.contains(e)){this.children_ar.splice(FWDSISCUtils.indexOfArray(this.children_ar, e), 1);}	
